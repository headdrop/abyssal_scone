...
<component :is="link ? MkA : 'span'" v-user-preview="preview ? user.id : undefined" v-bind="bound" class="_noSelect" :class="[$style.root, { [$style.animation]: animation, [$style.cat]: user.isCat, [$style.square]: squareAvatars }]" :style="{ color }" :title="acct(user)" @click.stop="onClick">
	<MkImgWithBlurhash :class="$style.inner" :src="(defaultStore.state.deidentifyMutedUsers && user.isMuted) ? (instance.iconUrl || '/favicon.ico') : url" :hash="user.avatarBlurhash" :cover="true" :onlyAvgColor="true"/>
	<MkUserOnlineIndicator v-if="!(defaultStore.state.deidentifyMutedUsers && user.isMuted) && indicator" :class="$style.indicator" :user="user"/>
	<div v-if="!(defaultStore.state.deidentifyMutedUsers && user.isMuted) && user.isCat" :class="[$style.ears]">
